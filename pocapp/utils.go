...
func NewPocAppUNSAFE(logger log.Logger, db dbm.DB, traceStore io.Writer, loadLatest bool,
	invCheckPeriod uint, baseAppOptions ...func(*baseapp.BaseApp),
) (gapp *PocApp, keyMain, keyStaking *sdk.KVStoreKey, stakingKeeper staking.Keeper) {

	gapp = NewPocApp(logger, db, traceStore, loadLatest, invCheckPeriod, baseAppOptions...)
...
func GenMintGenesisState(cdc *codec.Codec, r *rand.Rand, ap simulation.AppParams, genesisState map[string]json.RawMessage) {
	mintGenesis := mint.NewGenesisState(
		mint.InitialMinter(0,
			func(r *rand.Rand) sdk.Dec {
				var v sdk.Dec
				ap.GetOrGenerate(cdc, simulation.InitInflationAmount, &v, r,
					func(r *rand.Rand) {
						v = simulation.ModuleParamSimulator[simulation.InitInflationAmount](r).(sdk.Dec)
					})
				return v
			}(r),
		),
		mint.NewParams(
			...
			func(r *rand.Rand) []sdk.Dec {
				...
				var vset []sdk.Dec
				for i := 0; i < 5; i++ {
					ap.GetOrGenerate(cdc, simulation.InflactionFactors, &v, r,
						func(r *rand.Rand) {
							v = simulation.ModuleParamSimulator[simulation.InflactionFactors](r).(sdk.Dec)
						})
					vset = append(vset, v)
				}
				return vset
