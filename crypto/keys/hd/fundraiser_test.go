...
//Keep, read mnemonic from test.json, then generate testvetor according FundraiserPath
func TestFundraiserGenerateTestVectorsWithHD(t *testing.T) {
	...
	var testVectors []addrData
	for _, d := range hdToAddrTable {
		seed := bip39.NewSeed(d.Mnemonic, "")
		master, ch := ComputeMastersFromSeed(seed)
		priv, err := DerivePrivateKeyForPath(master, ch, "44'/11111'/0'/0/0")
		require.Nil(t, err)
		...
		testVector := addrData{
			...
			Master:   hex.EncodeToString([]byte(master[:])),
			Priv:     hex.EncodeToString([]byte(priv[:])),
			Pub:      hex.EncodeToString(pub.Bytes()[:]),
	...
	bz, err := json.MarshalIndent(testVectors, "", "")
	require.Nil(t, err)

	ioutil.WriteFile("testvector.json", bz, 0x777)
	t.Logf("%+v", testVectors)

}

func initFundraiserTestVectors(t *testing.T) []addrData {
	// NOTE: poc fundraiser address
	// var hdPath string = "m/44'/11111'/0'/0/0"
	var hdToAddrTable []addrData

	b, err := ioutil.ReadFile("testvector.json")
	if err != nil {
		t.Fatalf("could not read fundraiser test vector file (test.json): %s", err)
	}
...
func TestFundraiserCompatibility(t *testing.T) {
	...
	for i, d := range hdToAddrTable {
		...
		require.Equal(t, pub.Bytes(), pubB, fmt.Sprintf("Expected pub keys to match for %d", i))
