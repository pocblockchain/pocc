...
import (
	...
	"github.com/pocblockchain/pocc/crypto/keys/mintkey"
...
func TestCreateLedger(t *testing.T) {
	...
	ledger, err := kb.CreateLedger("some_account", Secp256k1, "poc", 3, 1)
	...
	assert.Equal(t, "pocpub1addwnpepqve34726hskzg9zk8c7j8jh2xk0j2stjd8k0tp084vnhs67smul0v5x58kh", pk)
	...
	assert.NoError(t, err)
	assert.Equal(t, "44'/11111'/3'/0/1", path.String())
}
...
func TestKeyManagement(t *testing.T) {
	...
	require.NoError(t, err)
	addr, err := sdk.AccAddressFromBech32("poc1kkkjfhv7t4swrurnftedsx0lngvc523cfa00s3")
	require.NoError(t, err)
...
func TestDerivedKey(t *testing.T) {
	kb := NewInMemory()
	bz, err := hex.DecodeString("502541d6b4ebc6f2928db9f385548d46965e702746998a9ffe2be7504a99b2a7")
	if err != nil {
		return
	}

	var derivedPriv [32]byte
	copy(derivedPriv[:], bz)
	//privKey := secp256k1.PrivKeySecp256k1(derivedPriv)
	// if we have a password, use it to encrypt the private key and store it
	// else store the public key only
	pubk := secp256k1.PrivKeySecp256k1(derivedPriv).PubKey()
	info, err := kb.CreateOffline("xxx", pubk)
	assert.Nil(t, err)
	t.Logf("address:%s", info.GetAddress())
	t.Logf("pubkey:%v", pubk)
}
