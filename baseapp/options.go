...
func SetMinGasPrices(gasPricesStr string) func(*BaseApp) {
	...
	if strings.Contains(gasPricesStr, ".") {
		gasPrices, err = sdk.ParseDecCoins(gasPricesStr)
		if err != nil {
			panic(fmt.Sprintf("invalid minimum gas prices decCoins: %v", err))
		}
	} else {
		gasPricesCoins, err := sdk.ParseCoins(gasPricesStr)
		if err != nil {
			panic(fmt.Sprintf("invalid minimum gas prices coins: %v", err))
		}
		gasPrices = sdk.NewDecCoins(gasPricesCoins)
	}
	return func(bap *BaseApp) { bap.setMinGasPrices(gasPrices.Sort()) }
}
