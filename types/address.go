...
const (
	...
	Bech32MainPrefix = "poc"

	// poc should be registered in https://github.com/satoshilabs/slips/blob/master/slip-0044.md
	CoinType = 11111

	// BIP44Prefix is the parts of the BIP44 HD path that are fixed by
	// what we used during the fundraiser.
	FullFundraiserPath = "44'/11111'/0'/0/0"
...
func AccAddressFromPubKey(pubKey crypto.PubKey) AccAddress {
	return AccAddress(pubKey.Address().Bytes())
}
func AccAddressFromByte(b []byte) AccAddress {
	if len(b) != AddrLen {
		return AccAddress{}
	}
	return AccAddress(b)
...
type SortAccAddress []AccAddress

func (s SortAccAddress) Len() int           { return len(s) }
func (s SortAccAddress) Swap(i, j int)      { s[i], s[j] = s[j], s[i] }
func (s SortAccAddress) Less(i, j int) bool { return bytes.Compare(s[i], s[j]) == -1 }
