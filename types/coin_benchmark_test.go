	...
	benchmarkingFunc := func(numCoinsA int, numCoinsB int) func(b *testing.B) {
		return func(b *testing.B) {
			...
			for i := 0; i < numCoinsA; i++ {
				coinsA[i] = NewCoin("coinz"+fmt.Sprintf("%v",i), NewInt(int64(i)))
			}
			for i := 0; i < numCoinsB; i++ {
				coinsB[i] = NewCoin("coinz"+fmt.Sprintf("%v",i), NewInt(int64(i)))
			}
...
func BenchmarkCoinsAdditionNoIntersect(b *testing.B) {
	benchmarkingFunc := func(numCoinsA int, numCoinsB int) func(b *testing.B) {
		return func(b *testing.B) {
			...
			for i := 0; i < numCoinsA; i++ {
				coinsA[i] = NewCoin("coinz"+fmt.Sprintf("%v",numCoinsB+i), NewInt(int64(i)))
			}
			for i := 0; i < numCoinsB; i++ {
				coinsB[i] = NewCoin("coinz"+fmt.Sprintf("%v",i), NewInt(int64(i)))
			}
