...
func (decs Decs) Len() int           { return len(decs) }
func (decs Decs) Less(i, j int) bool { return decs[i].LT(decs[j]) }
func (decs Decs) Swap(i, j int)      { decs[i], decs[j] = decs[j], decs[i] }
...
func DecMedian(decs Decs) Dec {
	...
	return (decs[half-1].Add(decs[half])).Quo(NewDec(2))
}
