...
func setupTestInput(t *testing.T) testInput {
	...
	bankKeeper := bank.NewBaseKeeper(accountKeeper, nil, paramsKeeper.Subspace(bank.DefaultParamspace), bank.DefaultCodespace, blacklistedAddrs)
	...
	mintKeeper.SetMinter(ctx, types.DefaultInitialMinter(types.DefaultParams().InitalInflationAmount))
