...
func TestHandleDoubleSign(t *testing.T) {
	...
	keeper.HandleValidatorSignature(ctx, val.Address(), sdk.TokensToConsensusPower(amt), true)
	...
	ctx = ctx.WithBlockTime(ctx.BlockTime().Add(staking.DefaultFrozenTime))
...
func TestHandleAbsentValidator(t *testing.T) {
	...
	slashAmt := amt.ToDec().Mul(keeper.SlashFractionDowntime(ctx)).TruncateInt()
