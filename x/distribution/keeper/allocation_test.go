...
func TestAllocateTokensToManyValidators(t *testing.T) {
	...
	//Check the distribution Module Account
	require.Equal(t, fees, supplyKeeper.GetModuleAccount(ctx, types.ModuleName).GetCoins())
	t.Logf("distribute Account:%v, fee pool:%v", supplyKeeper.GetModuleAccount(ctx, types.ModuleName).GetCoins(), k.GetFeePool(ctx).CommunityPool)
