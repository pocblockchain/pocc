...
func NewBaseKeeper(ak types.AccountKeeper, tk types.TokenKeeper,
	paramSpace params.Subspace,
	codespace sdk.CodespaceType, blacklistedAddrs map[string]bool) BaseKeeper {

	ps := paramSpace.WithKeyTable(types.ParamKeyTable())
	return BaseKeeper{
		BaseSendKeeper: NewBaseSendKeeper(ak, tk, ps, codespace, blacklistedAddrs),
		ak:             ak,
		paramSpace:     ps,
	}
}
...
// SetTokenKeeper set the tokenKeeper
func (keeper *BaseKeeper) SetTokenKeeper(tokenKeeper types.TokenKeeper) {
	keeper.tk = tokenKeeper
}
...
type SendKeeper interface {
	...
	EscrowCoins(ctx sdk.Context, fromAddr sdk.AccAddress, toAddr sdk.AccAddress, amt sdk.Coins) sdk.Error
	...
	IsCoinsSendEnabled(ctx sdk.Context, coins sdk.Coins) bool
}
...
func NewBaseSendKeeper(ak types.AccountKeeper, tk types.TokenKeeper,
...
//IsCoinsSendEnabled check whether coins are supported and sendenable in token module
func (keeper BaseSendKeeper) IsCoinsSendEnabled(ctx sdk.Context, coins sdk.Coins) bool {
	if keeper.tk == nil {
		panic("tokenKeeper is not set")
	}

	for _, coin := range coins {
		if !keeper.tk.IsSendEnabled(ctx, sdk.Symbol(coin.Denom)) {
			return false
		}
	}
	return true
...
// EscrowCoins escrow coins from one account to another
func (keeper BaseSendKeeper) EscrowCoins(ctx sdk.Context, fromAddr sdk.AccAddress, toAddr sdk.AccAddress, amt sdk.Coins) sdk.Error {
	_, err := keeper.SubtractCoins(ctx, fromAddr, amt)
	if err != nil {
		return err
	}
