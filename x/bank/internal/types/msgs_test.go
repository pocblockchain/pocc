...
func TestMsgSendGetSignBytes(t *testing.T) {
	...
	expected := `{"type":"poc/MsgSend","value":{"amount":[{"amount":"10","denom":"atom"}],"from_address":"poc1d9h8qat520tl9h","to_address":"poc1da6hgur4wsyj4tqc"}}`
...
func TestMsgMultiSendGetSignBytes(t *testing.T) {
	...
	expected := `{"type":"poc/MsgMultiSend","value":{"inputs":[{"address":"poc1d9h8qat520tl9h","coins":[{"amount":"10","denom":"atom"}]}],"outputs":[{"address":"poc1da6hgur4wsyj4tqc","coins":[{"amount":"10","denom":"atom"}]}]}}`
	require.Equal(t, expected, string(res))
}
...
func TestMsgEscrowRoute(t *testing.T) {
	addr1 := sdk.AccAddress([]byte("from"))
	addr2 := sdk.AccAddress([]byte("to"))
	coins := sdk.NewCoins(sdk.NewInt64Coin("atom", 10))
	var msg = NewMsgEscrow(addr1, addr2, coins)

	require.Equal(t, msg.Route(), RouterKey)
	require.Equal(t, msg.Type(), "escrow")
}

func TestMsgEscrowValidation(t *testing.T) {
	addr1 := sdk.AccAddress([]byte("from"))
	addr2 := sdk.AccAddress([]byte("to"))
	atom123 := sdk.NewCoins(sdk.NewInt64Coin("atom", 123))
	atom0 := sdk.NewCoins(sdk.NewInt64Coin("atom", 0))
	...
	cases := []struct {
		...
		{true, NewMsgEscrow(addr1, addr2, atom123)},       // valid send
		{true, NewMsgEscrow(addr1, addr2, atom123eth123)}, // valid send with multiple coins
		{false, NewMsgEscrow(addr1, addr2, atom0)},        // non positive coin
		{false, NewMsgEscrow(addr1, addr2, atom123eth0)},  // non positive coin in multicoins
		{false, NewMsgEscrow(emptyAddr, addr2, atom123)},  // empty from addr
		{false, NewMsgEscrow(addr1, emptyAddr, atom123)},  // empty to addr
	}
...
func TestMsgEscrowGetSignBytes(t *testing.T) {
	addr1 := sdk.AccAddress([]byte("input"))
	addr2 := sdk.AccAddress([]byte("output"))
	coins := sdk.NewCoins(sdk.NewInt64Coin("atom", 10))
	var msg = NewMsgEscrow(addr1, addr2, coins)
	...
	expected := `{"type":"poc/MsgEscrow","value":{"amount":[{"amount":"10","denom":"atom"}],"from_address":"poc1d9h8qat520tl9h","to_address":"poc1da6hgur4wsyj4tqc"}}`
	require.Equal(t, expected, string(res))
}

func TestMsgEscrowGetSigners(t *testing.T) {
	addr1 := sdk.AccAddress([]byte("input"))
	addr2 := sdk.AccAddress([]byte("output"))
	coins := sdk.NewCoins(sdk.NewInt64Coin("atom", 10))
	var msg = NewMsgEscrow(addr1, addr2, coins)
...
func TestMsgReclaimRoute(t *testing.T) {
	...
	addr2 := sdk.AccAddress([]byte("to"))
	coins := sdk.NewCoins(sdk.NewInt64Coin("atom", 10))
	var msg = NewMsgReclaim(addr1, addr2, coins)

	require.Equal(t, msg.Route(), RouterKey)
	require.Equal(t, msg.Type(), "reclaim")
}
...
func TestMsgReclaimValidation(t *testing.T) {
	...
	cases := []struct {
		...
		{true, NewMsgReclaim(addr1, addr2, atom123)},       // valid send
		{true, NewMsgReclaim(addr1, addr2, atom123eth123)}, // valid send with multiple coins
		{false, NewMsgReclaim(addr1, addr2, atom0)},        // non positive coin
		{false, NewMsgReclaim(addr1, addr2, atom123eth0)},  // non positive coin in multicoins
		{false, NewMsgReclaim(emptyAddr, addr2, atom123)},  // empty from addr
		{false, NewMsgReclaim(addr1, emptyAddr, atom123)},  // empty to addr
	}
...
func TestMsgReclaimGetSignBytes(t *testing.T) {
	...
	expected := `{"type":"poc/MsgReclaim","value":{"amount":[{"amount":"10","denom":"atom"}],"from_address":"poc1d9h8qat520tl9h","to_address":"poc1da6hgur4wsyj4tqc"}}`
	require.Equal(t, expected, string(res))
}
...
func TestMsgBonusSendRoute(t *testing.T) {
	// Construct a MsgSend
	addr1 := sdk.AccAddress([]byte("input"))
	addr2 := sdk.AccAddress([]byte("output"))
	coins := sdk.NewCoins(sdk.NewInt64Coin("atom", 10))
	var msg = MsgBonusSend{
		Inputs:  []Input{NewInput(addr1, coins)},
		Outputs: []Output{NewOutput(addr2, coins)},
	}
	...
	// TODO some failures for bad result
	require.Equal(t, msg.Route(), RouterKey)
	require.Equal(t, msg.Type(), "bonus_send")
}

func TestMsgBonusSendValidation(t *testing.T) {
	addr1 := sdk.AccAddress([]byte{1, 2})
	addr2 := sdk.AccAddress([]byte{7, 8})
	atom123 := sdk.NewCoins(sdk.NewInt64Coin("atom", 123))
	atom124 := sdk.NewCoins(sdk.NewInt64Coin("atom", 124))
	...
	cases := []struct {
		...
		{false, MsgBonusSend{}},                           // no input or output
		{false, MsgBonusSend{Inputs: []Input{input1}}},    // just input
		{false, MsgBonusSend{Outputs: []Output{output1}}}, // just output
		{false, MsgBonusSend{
			Inputs:  []Input{NewInput(emptyAddr, atom123)}, // invalid input
			Outputs: []Output{output1}}},
		{false, MsgBonusSend{
...
func TestMsgBonusSendGetSignBytes(t *testing.T) {
	addr1 := sdk.AccAddress([]byte("input"))
	addr2 := sdk.AccAddress([]byte("output"))
	coins := sdk.NewCoins(sdk.NewInt64Coin("atom", 10))
	var msg = MsgBonusSend{
	...
	expected := `{"type":"poc/MsgBonusSend","value":{"inputs":[{"address":"poc1d9h8qat520tl9h","coins":[{"amount":"10","denom":"atom"}]}],"outputs":[{"address":"poc1da6hgur4wsyj4tqc","coins":[{"amount":"10","denom":"atom"}]}]}}`
	require.Equal(t, expected, string(res))
}

func TestMsgBonusSendGetSigners(t *testing.T) {
	var msg = MsgBonusSend{
		Inputs: []Input{
			NewInput(sdk.AccAddress([]byte("input1")), nil),
			NewInput(sdk.AccAddress([]byte("input2")), nil),
...
func TestMsgReclaimSendRoute(t *testing.T) {
	// Construct a MsgSend
	addr1 := sdk.AccAddress([]byte("input"))
	addr2 := sdk.AccAddress([]byte("output"))
	coins := sdk.NewCoins(sdk.NewInt64Coin("atom", 10))
	var msg = MsgReclaimSend{
		Inputs:  []Input{NewInput(addr1, coins)},
		Outputs: []Output{NewOutput(addr2, coins)},
	}
	...
	// TODO some failures for bad result
	require.Equal(t, msg.Route(), RouterKey)
	require.Equal(t, msg.Type(), "reclaim_send")
}

func TestMsgReclaimSendValidation(t *testing.T) {
	addr1 := sdk.AccAddress([]byte{1, 2})
	addr2 := sdk.AccAddress([]byte{7, 8})
	atom123 := sdk.NewCoins(sdk.NewInt64Coin("atom", 123))
	atom124 := sdk.NewCoins(sdk.NewInt64Coin("atom", 124))
	...
	cases := []struct {
		...
		{false, MsgReclaimSend{}},                           // no input or output
		{false, MsgReclaimSend{Inputs: []Input{input1}}},    // just input
		{false, MsgReclaimSend{Outputs: []Output{output1}}}, // just output
		{false, MsgReclaimSend{
			Inputs:  []Input{NewInput(emptyAddr, atom123)}, // invalid input
			Outputs: []Output{output1}}},
		{false, MsgReclaimSend{
			Inputs:  []Input{input1},
...
func TestMsgReclaimSendGetSignBytes(t *testing.T) {
	addr1 := sdk.AccAddress([]byte("input"))
	addr2 := sdk.AccAddress([]byte("output"))
	coins := sdk.NewCoins(sdk.NewInt64Coin("atom", 10))
	var msg = MsgReclaimSend{
	...
	expected := `{"type":"poc/MsgReclaimSend","value":{"inputs":[{"address":"poc1d9h8qat520tl9h","coins":[{"amount":"10","denom":"atom"}]}],"outputs":[{"address":"poc1da6hgur4wsyj4tqc","coins":[{"amount":"10","denom":"atom"}]}]}}`
	require.Equal(t, expected, string(res))
}

func TestMsgReclaimSendGetSigners(t *testing.T) {
	var msg = MsgReclaimSend{
		Inputs: []Input{
			NewInput(sdk.AccAddress([]byte("input1")), nil),
			NewInput(sdk.AccAddress([]byte("input2")), nil),
