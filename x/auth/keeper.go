...
import (
	...
	"github.com/pocblockchain/pocc/x/params/subspace"
)
...
func (ak AccountKeeper) GetOrNewAccount(ctx sdk.Context, addr sdk.AccAddress) exported.Account {
	store := ctx.KVStore(ak.key)
	bz := store.Get(types.AddressStoreKey(addr))
	if bz == nil {
		return ak.NewAccountWithAddress(ctx, addr)
	}
	acc := ak.decodeAccount(bz)
	return acc
}
