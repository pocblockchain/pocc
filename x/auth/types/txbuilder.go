...
import (
	...
	crkeys "github.com/pocblockchain/pocc/crypto/keys"
...
func (bldr TxBuilder) WithGasPrices(gasPrices string) TxBuilder {
	...
	if strings.Contains(gasPrices, ".") {
		prices, err = sdk.ParseDecCoins(gasPrices)
		if err != nil {
			panic(fmt.Sprintf("invalid gas prices decCoins: %v", err))
		}
	} else {
		gasPricesCoins, err := sdk.ParseCoins(gasPrices)
		if err != nil {
			panic(fmt.Sprintf("invalid gas prices coins: %v", err))
		}
		prices = sdk.NewDecCoins(gasPricesCoins)
	}

	bldr.gasPrices = prices
	return bldr
}
