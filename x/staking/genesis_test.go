...
func TestInitGenesisLargeValidatorSet(t *testing.T) {
	...
	params := keeper.GetParams(ctx)
	maxValidators := uint16(size)
	if params.MaxValidators < maxValidators {
	...
	abcivals := make([]abci.ValidatorUpdate, maxValidators)
	for i, val := range validators[:maxValidators] {
		abcivals[i] = val.ABCIValidatorUpdate()
	}
