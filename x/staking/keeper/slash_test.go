...
func setupHelper(t *testing.T, power int64) (sdk.Context, Keeper, types.Params) {
	...
	for i := int64(0); i < numVals; i++ {
		...
		validator.CreateTime = ctx.BlockTime()
...
func TestSlashWithUnbondingDelegation(t *testing.T) {
	...
	ubd := types.NewUnbondingDelegation(addrDels[0], addrVals[0], 11,
		ctx.BlockTime().Add(1*time.Second), ubdTokens)
	keeper.SetUnbondingDelegation(ctx, ubd)
	...
	// this may not be the desirable behaviour, ref https://github.com/pocblockchain/pocc/issues/1440
	ctx = ctx.WithBlockHeight(13)
	keeper.Slash(ctx, consAddr, 9, 10, fraction)
	ubd, found = keeper.GetUnbondingDelegation(ctx, addrDels[0], addrVals[0])
	require.True(t, found)
...
func TestSlashWithRedelegation(t *testing.T) {
	...
	rd := types.NewRedelegation(addrDels[0], addrVals[0], addrVals[1], 11,
		ctx.BlockTime().Add(1*time.Second), rdTokens, rdTokens.ToDec())
	keeper.SetRedelegation(ctx, rd)
...
func TestSlashBoth(t *testing.T) {
	...
	rdA := types.NewRedelegation(addrDels[0], addrVals[0], addrVals[1], 11,
		ctx.BlockTime().Add(1*time.Second), rdATokens,
		rdATokens.ToDec())
	...
	ubdA := types.NewUnbondingDelegation(addrDels[0], addrVals[0], 11,
		ctx.BlockTime().Add(1*time.Second), ubdATokens)
	...
	// slash validator
	ctx = ctx.WithBlockHeight(12)
	ctx = ctx.WithBlockTime(ctx.BlockTime())
