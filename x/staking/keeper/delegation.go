...
func (k Keeper) Undelegate(
	...
	//can not undelegate operator's share before 1 year's frozen period
	if delAddr.Equals(sdk.AccAddress(valAddr)) &&
		ctx.BlockHeader().Time.Before(validator.CreateTime.Add(types.DefaultFrozenTime)) {
		return time.Time{}, types.ErrStillInFrozen(k.Codespace())
	}
...
func (k Keeper) BeginRedelegation(ctx sdk.Context, delAddr sdk.AccAddress,
	...
	//can not redelegate operator's share before 1 year's frozen period
	if delAddr.Equals(sdk.AccAddress(valSrcAddr)) &&
		ctx.BlockHeader().Time.Before(srcValidator.CreateTime.Add(types.DefaultFrozenTime)) {
		return time.Time{}, types.ErrStillInFrozen(k.Codespace())
	}
