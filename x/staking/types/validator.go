...
type Validator struct {
	...
	CreateTime              time.Time      `json:"create_time" yaml:"create_time"`                 // validator's create time
}
...
func NewValidator(operator sdk.ValAddress, pubKey crypto.PubKey, description Description) Validator {
	return Validator{
		...
		CreateTime:              time.Unix(0, 0).UTC(), //should be assigned immediately
	}
}
...
func (v Validator) String() string {
  ...
  Commission:                 %s
  Create Time:                %s`, v.OperatorAddress, bechConsPubKey,
		v.Jailed, v.Status, v.Tokens,
		v.DelegatorShares, v.Description,
		v.UnbondingHeight, v.UnbondingCompletionTime, v.MinSelfDelegation, v.Commission, v.CreateTime)
}
...
type bechValidator struct {
	...
	CreateTime              time.Time      `json:"create_time" yaml:"create_time"`                 // create time
}
...
// SetCreateTime attempts to set a validator's create time. An
// error is returned if the commission is invalid.
func (v Validator) SetCreateTime(createAt time.Time) Validator {
	v.CreateTime = createAt
	return v
}
...
func (v Validator) GetCreatTime() time.Time       { return v.CreateTime }
